...
Blob publication uses same-filesystem `Files.createLink(final, staged)` as the atomic create-if-absent publication point, followed by unlinking the staging name. Before linking, the verified staged blob is `fsync`ed and changed to mode `0444`; the hard link therefore exposes the same read-only inode. An existing destination is reused only after exact no-follow regular-file, size, permission, and SHA-256 verification. Unsupported hard links and cross-device linking are `ATOMIC_PUBLISH_UNSUPPORTED`; copy, replacing rename, and non-atomic fallback are forbidden.
